...
/**
 * A hideous but effective class for replacing tone numbers with with tone marks in pinyin.
 * @author Admin
 */
public class PinyinReplacer {
	 ...
	 */
	public static String convertNumberedToneMarksToVisual(String input) {
		if (input.contains(" ")) {
	 ...
	 */
	public static String removeNumericToneMarks(String input) {
		return input.replaceAll("\\d", "");
	}
	
	public static String removeVisualToneMarks(String string) {
		for (int i = 0; i < replacementTable.length; i++) {
			if (string.contains(replacementTable[i][1])) {
				string = string.replace(replacementTable[i][1], replacementTable[i][0]);
			}
		}
		return string;
	}
	
	public static String removeAllToneMarks(String string) {
		return removeNumericToneMarks(removeVisualToneMarks(string)).trim();
	}
}
